...
class ScalaSuite extends CatsEffectSuite {
  test("Type class v1") {
    ...
    implicit class AreaOps[S: Area](s: S) { // extension
  ...
  test("Type class v2") {
    ...
    def halfUp(d: Double) = BigDecimal.apply(d).setScale(2, RoundingMode.HALF_UP).doubleValue

    implicit object SquareArea extends Area[Square] {
      override def calculate(s: Square): Double = halfUp(Math.pow(s.a, 2.0))
    }

    implicit object RectangleArea extends Area[Rectangle] {
      override def calculate(s: Rectangle): Double = halfUp(s.a * s.b)
    }

    implicit object CircleArea extends Area[Circle] {
      override def calculate(s: Circle): Double = halfUp(s.r * Math.PI)
    }

    def area[T](shape: T)(implicit area: Area[T]): Double = area.calculate(shape)
    def area2[T: Area](shape: T): Double                  = implicitly[Area[T]].calculate(shape)

    assertEquals(area(Square(5.2)), 27.04)
    assertEquals(area(Rectangle(5.2, 2.54)), 13.21)
    assertEquals(area(Circle(2.3)), 7.23)

    assertEquals(area2(Square(5.2)), 27.04)
    assertEquals(area2(Rectangle(5.2, 2.54)), 13.21)
    assertEquals(area2(Circle(2.3)), 7.23)
  }
